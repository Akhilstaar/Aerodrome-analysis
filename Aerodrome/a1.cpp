...
void Aerodrome(const vector<string> &tracedata) {
   ...
   // We assume that the maximum number of threads is known and stored in a constant called maxThreads.
    ...
    vector<int> possible_sizes = {1, 2, 4, 8, 16, 32}; // hard-coded for now
    ...
    ofstream Aerodrome_out_file("Aerodrome_output.log");

    if (!Aerodrome_out_file) {
        cerr << "error while opening Aerodrome_output.log" << endl;
        return;
    }

    for(const auto &ele: dr){
        Aerodrome_out_file << ele.first << " Count: " << ele.second << endl;
    }

    Aerodrome_out_file.close();
    cout << "Aerodrome+ output saved to 'Aerodrome_output.log'\n";
