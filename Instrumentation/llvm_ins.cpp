...
struct ExtendedInstrumentationPass : public PassInfoMixin<ExtendedInstrumentationPass> {
  ...
  PreservedAnalyses run(Module &M, ModuleAnalysisManager &) {
    ...
    // Declare the external logging function:
    // void logEvent(int eventType, void* addr);
        ...
        for (Instruction *I : targets) {
          ...
          if (auto *loadInst = dyn_cast<LoadInst>(I)) {
              ...
              if (!loadInst->isAtomic()) {
                  ...
                  Value *castedPtr = builder.CreateBitCast(ptr, Type::getInt8PtrTy(Context));
                  Value *eventType = ConstantInt::get(Type::getInt32Ty(Context), READ_EVENT);
                  builder.CreateCall(logFunc, {eventType, castedPtr});
              }
              ...
              if (Function *called = CI->getCalledFunction()) {
                  ...
                  if (funcName == "pthread_mutex_lock") {
                      if (CI->arg_size() >= 1) {
                          ...
                          Value *castedPtr = builder.CreateBitCast(mutexArg, Type::getInt8PtrTy(Context));
                  ...
                  // For thread creation, log a fork event.
                      ...
                      Value *nullPtr = Constant::getNullValue(Type::getInt8PtrTy(Context));
                      Value *eventType = ConstantInt::get(Type::getInt32Ty(Context), FORK_EVENT);
                      builder.CreateCall(logFunc, {eventType, nullPtr});
                  } 
